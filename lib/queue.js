...
var Queue = function Queue(name, url, opts) {
  ...
  var lazyClient = redisClientGetter(this, opts, function(type, client) {
    ...
    if (type === 'client') {
      ...
      if (_this.settings.expiresIn) {
        client.config("SET", "notify-keyspace-events", "Ex", function(err, result) {
          client.psubscribe("*:expired", function(err, result) {
            client.on('pmessage', function(pattern, channel, message) {
              var keyAndToken = channel.split('@');
              var token = keyAndToken[1];
              if (token == "0__:expired") {
                var jobId = _this.getJobIdFromJobKey(message);
                if (jobId && typeof jobId == "number") {
                  scripts.remove(_this, jobId);
                }
              }
            });
          });
...
Queue.prototype.getJobIdFromJobKey = function(jobKey) {
  return parseInt(jobKey.replace([this.keyPrefix, this.name, ''].join(':'), ''));
}
