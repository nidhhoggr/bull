...
if rcall("EXISTS", KEYS[3]) == 1 then -- // Make sure job exists
  ...
  if ARGV[6] == "1" then
    ...
    if string.match(KEYS[2], ":completed") and ARGV[7] ~= "0" then
      rcall("EXPIRE", KEYS[3], ARGV[7]) 
    end
  end

  rcall("PUBLISH", KEYS[2], ARGV[8])
  ...
  if(ARGV[9] == "1") then
    ...
    if jobId then
      local jobKey = ARGV[10] .. jobId
      local lockKey = jobKey .. ':lock'

      -- get a lock
      rcall("SET", lockKey, ARGV[12], "PX", ARGV[11])
